...
### 7. Playback holds a 800 ms cushion, then starts

See [Why the pre-roll exists](#why-the-pre-roll-exists). This is the least intuitive step and the one most worth understanding.
...
:::warning Lowering the pre-roll needs faster inference, not just a smaller number
300 ms was tested on this hardware and **underran**. The cushion is not a comfort margin — it is sized directly from the measured deficit. Reducing it without changing the provider brings back mid-sentence dropouts.

The real fix is inference headroom. At the RTF ~0.3 the Qwen3 docs quote (4090, 0.6B model), a 300 ms cushion would be ample and first audio would land near 350 ms. On an L4 with the 1.7B model there is no headroom to stream into.
:::
...
|---|---|---|
| `STELLA_TTS_PREROLL_MS` | `800` | Jitter buffer held before the first frame of an utterance. Read once at pipeline construction. `0` disables it — only sane if the provider streams comfortably faster than real time. **See the warning above before lowering.** |
