...
from stella_v2_agent.agent import _TURN_SPEED_BY_BRIDGE_TYPE, _turn_speed
...
def test_ordinary_turn_is_the_reference_rate():
    assert _turn_speed(BRIDGE_TYPE_ACKNOWLEDGEMENT) == 1.0


def test_a_heavier_turn_is_taken_slower():
    assert _turn_speed(BRIDGE_TYPE_PENSIVE) < 1.0


def test_an_opening_beat_is_brisker():
    assert _turn_speed(BRIDGE_TYPE_GREETING) > 1.0
    assert _turn_speed(BRIDGE_TYPE_MINIMAL) > 1.0
...
def test_variation_stays_within_a_few_percent():
    ...
    """
    for rate in _TURN_SPEED_BY_BRIDGE_TYPE.values():
...
def test_the_range_is_actually_used():
    ...
    assert len(set(_TURN_SPEED_BY_BRIDGE_TYPE.values())) > 1


@pytest.mark.parametrize("unknown", [None, BRIDGE_TYPE_NONE, "not-a-type"])
def test_unknown_types_fall_back_to_the_reference_rate(unknown):
...
def test_kill_switch_disables_variation(monkeypatch):
    ...
    for bridge_type in _TURN_SPEED_BY_BRIDGE_TYPE:
        assert _turn_speed(bridge_type) == 1.0
